...
prepare_data_web<-function(){
  
}


fromJSON("https://www.necc.mass.edu/courses/api/services.php")%>% View()

$message %>% str_split(.,"wp_all")
...
%>%
  ...
  flatten_dfr() %>% 
  select(-id,-term) %>% 
  unnest(schoolClasses)%>% 
  mutate(day=str_extract_all(days,
                             "(\\w+)(?=[\\]])|(?<=[\\[])"))%>% 
  unnest_longer(day) %>%
  mutate(day=fct_relevel(day,c("M","T","W","R","F","S"))) %>%
  mutate(startTime=parse_date_time(startTime,'%I:%M %p'),
         endTime=parse_date_time(endTime,'%I:%M %p')) %>%
  unite(col=abvTitle,c("abvTitle","section"),sep=" ") %>%
  mutate(stime=hour(startTime)+minute(startTime)/60,
         etime=hour(endTime)+minute(endTime)/60) %>%
  mutate(p=(etime-stime)/2) %>%
  mutate(campus=case_when(campus == "" ~ room,
                          TRUE ~ as.character(campus)))->main_sched


"https://www.necc.mass.edu/courses/api/ssbconnector.php?skey=1587665626&course=202005-GOV101"
    
library(httr)

d<-POST("https://www.necc.mass.edu/courses/api/services.php",content_type_json()) ### get the data
content(d,as = "text") %>% fromJSON() %>% .$data %>% flatten_dfr() -> data

info<-GET("https://www.necc.mass.edu/courses/api/ssbconnector.php?skey=1587665626&course=202005-GOV101")



data %>% select(term,abvTitle)

content(info) %>% rvest::html_nodes("#main") %>% rvest::html_text()
  
content(info,"parsed") %>% rvest::html_node(xpath = "/text")

xml2::read_html("https://necc.smartcatalogiq.com/2020-2021/Catalog/Courses/GOV-Government/100/GOV101") %>%
  rvest::html_nodes(xpath = "//h3/following-sibling::text()") %>% rvest::html_text(.,trim = TRUE) %>% View()
